...
- State management changes:
  ...
  - [x] Shared composable view-model layer for members/groups screens
...
- frontend/src/composables/useTeamDirectoryViewModel.ts: Shared composable view-model functions for members/groups pages
- frontend/src/composables/useMemberDetailsEditorModel.ts: Dedicated composable for member editor local form state/validation/actions
...
- README.md: Root project overview and setup documentation for repository landing page
...
3. Backend includeDeleted semantics return active-only when false and deleted-only when true.
4. Create/edit/delete/undelete actions call corresponding store methods, then refresh member list while preserving includeDeleted filter.
...
- Edit state: Opening create/edit disables top-level member list controls to avoid conflicting state transitions.
- Visibility state: Member list heading displays a live row count based on current loaded dataset.
- Navigation safety state: Route transitions from members/groups prompt before discarding unsaved form/editor state.
- Row safety state: Members row edit/delete actions are blocked while an editor is already open.
...
| View script growth and duplication     | Medium | Centralize view logic in a shared composable and keep SFC scripts as thin wiring only  |
| Accidental loss during navigation/edit | Medium | Add route-leave confirmations and disable conflicting row actions while editor is open |
...
| AC-013 | Slide-down inline editor row component with cancel rollback | In-place editing UX with disabled top-level controls         |
